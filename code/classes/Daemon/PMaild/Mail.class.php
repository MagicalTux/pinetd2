...
class Mail {
	...
	private $sql;
	const MIME_CACHE_MAGIC = 0xb0ca5;
		...
		foreach($struct as $part) {
			...
			if (($type == 'multipart') && ($part == '1')) {
				...
				if (is_null($parent)) {
					...
					$imap_part .= ($imap_part == '' ? '' : '.').$imap_count[$cur_depth_parent];
				}
			}
		...
		for($i=0;$i<$len;$i++) {
			...
			switch(strtoupper($p)) {
				...
				default:
					...
					$only_header = false;
					$p_search = $p;
					if (strtoupper(substr($p_search, -5)) == '.MIME') {
						$only_header = true;
					...
					if ($part) {
						...
						if ($only_header) {
							...
							$data_len = $part->starting_pos_body - $part->starting_pos;
						} else {
